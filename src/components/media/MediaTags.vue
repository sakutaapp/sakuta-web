  ...
  <Section>
    ...
    <div class="flex flex-col items-stretch space-y-1">
      <Genre
        ...
        :key="tag.id"
        :text="tag.name"
        :rank="tag.rank.toString()"
        :spoiler="tag.isMediaSpoiler || tag.isGeneralSpoiler"
      />
      <p class="text-gray-500 text-sm cursor-pointer whitespace-nowrap" @click="showSpoilers = !showSpoilers">
        {{ $t("media.tags.toggleSpoilers") }}
      </p>
    </div>
  </Section>
</template>
