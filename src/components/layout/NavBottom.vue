<template>
  <div :class="`pointer-events-none fixed w-full bottom-5 left-0 z-100 flex justify-center ${alwaysShow ? '' : 'md:hidden'}`">
    <div class="bg-dark-800 flex justify-around items-center py-2 text-white space-x-4 px-3 rounded-full shadow-2xl pointer-events-auto">
      <nuxt-link
        to="/"
        class="opacity-40 nav-bottom-link flex items-center space-x-2 transition duration-250 bg-dark-800 hover:bg-dark-900 rounded-full px-4 py-2"
      >
        <HomeIcon :filled="$route.path === '/'" size="30" />
        <span class="font-medium text-lg hidden md:block">{{ $t("nav.link.home") }}</span>
      </nuxt-link>

      <div class="bg-primary rounded-full p-2 block md:hidden" @click="$emit('toggle-command-menu')">
        <SearchIcon size="30" :filled="true" />
      </div>
      ...
      <nuxt-link
        to="/news"
        class="opacity-40 nav-bottom-link flex items-center space-x-2 transition duration-250 bg-dark-800 hover:bg-dark-900 rounded-full px-4 py-2 :temp: hidden md:flex"
      >
