    ...
    <div>
        <div class="hidden md:block flex flex-col">
            <div class="flex items-center justify-between cursor-pointer" @click="handleToggle()">
                <p class="text-white font-medium">{{ $t(`settings.options.${name}`) }}</p>
                <div :class="`${active ? 'bg-primary' : 'bg-dark-900 bg-opacity-50'} h-7 w-12 rounded-full relative transition duration-250`">
                    <div :class="`absolute ${animate ? 'animate' : ''} ${active ? 'on' : 'off'} h-5 bg-white rounded-full`"></div>
                </div>
            </div>
            <p class="text-sm opacity-50" v-if="description">{{ $t(`settings.options.${name}.description`) }}</p>
        </div>
        <div class="md:hidden flex items-center" @click="handleToggle()">
            ...
            <div :class="`${active ? 'bg-primary' : 'bg-dark-800 bg-opacity-100'} h-7 w-12 min-w-12 rounded-full relative transition duration-250 flex-grow`">
                <div :class="`absolute ${animate ? 'animate' : ''} ${active ? 'on' : 'off'} h-5 bg-white rounded-full`"></div>
            </div>
        </div>
    </div>
