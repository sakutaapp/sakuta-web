<template>
    <nuxt-link :to="`/${type}/${id}`" class="media-card group">
        <div class="w-full aspect-w-3 aspect-h-3.5 media-card-cover rounded-lg overflow-hidden transition-all duration-250" :style="`background-image: url('${coverImage}')`">
            <div :class="`w-full h-full bg-dark-900 bg-opacity-70 backdrop-filter backdrop-blur-sm transition duration-250 p-2 md:p-3 lg:p-4 text-white text-opacity-80 media-card-cover-inner opacity-0 group-hover:opacity-100`">
                <h3 class="text-lg md:text-xl lg:text-2xl font-bold uppercase text-white text-opacity-100">{{ charLimit(title, 55) }}</h3>
                ...
                <h4 v-if="season && seasonYear" class="text-xs md:text-sm font-semibold uppercase">{{ $t(`media.season`, { season: $t(`media.season.${season}`), year: seasonYear }) }}</h4>
            </div>
        </div>
        <p class="mt-1 media-card-text transition duration-250">{{ charLimit(title, 45) }}</p>
    </nuxt-link>
</template>
