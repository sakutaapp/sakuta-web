    ...
    <nuxt-link :to="url">
        <div class="bg-gray-900 dark:bg-dark-900 px-5 py-3 flex rounded shadow hover:pl-6 cursor-pointer transition-all duration-250">
            ...
            <div class="flex-grow w-full self-center ml-5">
                <h3>{{ $tc(`activities.status.${status}`, plural ? 2 : 1, [progress]) }}</h3>
                <h2 class="text-xl">{{ title }}</h2>
            </div>
            <div class="flex flex-col justify-between text-right text-sm text-gray-500 flex-grow w-full">
                <p>{{ $moment.unix(date).locale($i18n.locale).fromNow() }}</p>
...
function toCamelCase(string: string): string {
    return string
        .replace(/\s(.)/g, function(a: string) { return a.toUpperCase(); })
        .replace(/\s/g, '')
...
export default Vue.extend({
    ...
    computed: {
        cover(): any { return this.activity?.media?.coverImage?.medium || "" },
        status(): any { return toCamelCase(this.activity?.status) || "" },
        plural(): any { return this.activity?.progress && this.activity.progress.includes(" - ") },
        progress(): any { return this.activity?.progress || undefined },
        title(): any { return this.activity?.media?.title?.english || this.activity?.media?.title?.romaji || this.activity?.media?.title?.native || "" },
        date(): any { return this.activity?.createdAt || "Unknown" },
        likeCount(): any { return this.activity?.likeCount || 0 },
        url(): any { return `/${this.activity?.media?.type?.toLowerCase()}/${this?.activity?.media?.id}` || "" }
    }
});
</script>