    ...
    <Container>
        <div class="bg-primary bg-opacity-15 rounded-lg flex space-x-3 p-3 items-center mt-3 mb-5">
            ...
            <div class="flex flex-col items-start">
                ...
                <p v-if="language !== 'en'" class="mt-2 font-medium">{{ $t("news.notReleased.text.language") }}</p>
                <Button v-if="language !== 'en'" to="/news?lang=en" css="bg-dark-900 hover:bg-dark-800 text-white mt-2">{{ $t("news.notReleased.viewInEnglish") }}</Button>
            </div>
        </div>
        ...
        <div v-else>
            <p class="text-center">{{ $t("news.loadingPosts") }}</p>
        </div>
    </Container>
</template>
...
export default Vue.extend({
    ...
    methods: {
        updatePosts() {
            ...
            this.pulled = false;
            this.$axios.$get(`https://cms.sakuta.app/items/articles?fields=slug,title,published,image.filename_disk,author.*,author.avatar.filename_disk&sort=-published&filter[language][_eq]=${this.language}`).then((response: any) => {
